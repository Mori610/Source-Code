...
train_df = train_df*1
for a in range(1,10):
    name = 'parts_{}'.format(a) 
    Direc = train_df[name].value_counts()
    Direc = Direc.to_dict()
    train_df[name] = pd.Series([Direc[i]  for i in train_df[name]])

for b in range(1,4):
    name = 'attribute_{}'.format(b) 
    Direc = train_df[name].value_counts()
    Direc = Direc.to_dict()
    train_df[name] = pd.Series([Direc[i]  for i in train_df[name]])

use_Direc = train_df['user_id'].value_counts()
#print(train_df['user_id'].value_counts())
use_Direc = use_Direc.to_dict()
train_df['user_id'] = pd.Series([use_Direc[a]  for a in train_df['user_id']])

use_Direc = train_df['product_id'].value_counts()
#print(train_df['product_id'].value_counts())
use_Direc = use_Direc.to_dict()
train_df['product_id'] = pd.Series([use_Direc[a]  for a in train_df['product_id']])
...
train = train_df.drop('purchase',axis =1)
...
for a in range(1,10):
    name = 'parts_{}'.format(a) 
    Direc = test_df[name].value_counts()
    Direc = Direc.to_dict()
    test_df[name] = pd.Series([Direc[i]  for i in test_df[name]])
...
use_Direc = test_df['user_id'].value_counts()
use_Direc = use_Direc.to_dict()
test_df['user_id'] = pd.Series([use_Direc[a]  for a in test_df['user_id']])

use_Direc = test_df['product_id'].value_counts()
use_Direc = use_Direc.to_dict()
test_df['product_id'] = pd.Series([use_Direc[a]  for a in test_df['product_id']])
...
proba = model_lgb.predict_proba(test)[:, 1]
...
submit_df.to_csv('AIQuest/submit_191128_2.csv', header=False, index=False, )
